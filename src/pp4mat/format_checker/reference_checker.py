...
class ReferenceChecker(FormatChecker):
    ...
    def reference_checker(
        ...
        en_cnt = 0
        for p in reference:
            if is_reference_entry(p):
                cnt += 1
                # 英文条目：沿用你原来的逻辑（全文 ascii），避免误判混排
                if all(char.isascii() for char in (p.text or "")):
                    en_cnt += 1

        bucket = errors.setdefault("参考文献检测", [])

        if cnt < config["min_count"]:
            msg = f"参考文献数量少于{config['min_count']}条，当前数量为{cnt}条，请检查！"
            logger.error(msg)
            bucket.append(f"参考文献数量少于{config['min_count']}条，当前数量为{cnt}条")
        else:
