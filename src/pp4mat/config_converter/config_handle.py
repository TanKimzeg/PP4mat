...
class FormatConfig:
    ...
    def reload(self) -> None:
        """将配置文件里的参数赋予单独变量，方便后续程序调用。"""
        configs = self._load_config()

        self.title_config: dict[str, Any] | None = configs.get('title', None)
        self.text_config: dict[str, Any] | None = configs.get('text', None)
        self.formula_config: dict[str, Any] | None = configs.get('formula', None)
        self.figure_config: dict[str, Any] | None = configs.get('figure', None)
        self.table_config: dict[str, Any] | None = configs.get('table', None)
        self.reference_config: dict[str, Any] | None = configs.get('reference', None)
        ...
        self.heading4_config: dict[str, Any] | None = configs.get('heading4', None)
        self.heading5_config: dict[str, Any] | None = configs.get('heading5', None)
        self.heading6_config: dict[str, Any] | None = configs.get('heading6', None)
...
class Config:
    def __init__(
        self,
        args: Args,
        converters: Iterable[Converter] | None = None,
        *,
        ...
        if auto_convert:
            convert_config(self.format_config, converters)
